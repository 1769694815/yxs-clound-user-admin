...
<template>
  <div class="app-container calendar-list-container">
    ...
    <el-form class="menu">
      <el-form-item>
        <el-button v-if="roleManager_btn_add" type="primary" icon="el-icon-edit" size="medium" @click="handleCreate">添加</el-button>
      </el-form-item>
    </el-form>
    <!-- 表格 -->
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border stripe highlight-current-row>
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-button v-if="roleManager_btn_edit" type="text" icon="el-icon-edit" size="mini" @click="handleUpdate(scope.row)">编辑</el-button>
          <el-button v-if="roleManager_btn_del" type="text" icon="el-icon-delete" size="mini" @click="handleDelete(scope.row)">删除</el-button>
          <el-button v-if="roleManager_btn_perm" type="text" icon="el-icon-plus" size="mini" @click="handlePermission(scope.row)">权限</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页器 -->
    ...
    <el-dialog :visible.sync="dialogPvVisible" :title="dialogTitle(operationStatus)">
        ...
        <el-form ref="dataForm" :rules="formRules" :model="form">
          <el-col>
            <el-form-item prop="roleName" label="角色名称:" :label-width="formLabelWidth">
              <el-input v-model="form.roleName" autocomplete="off" :disabled="operationStatus === 1" placeholder="请输入角色名称"></el-input>
            </el-form-item>
          </el-col>
          <el-col>
            <el-form-item prop="roleCode" label="角色标识:" :label-width="formLabelWidth">
              <el-input v-model="form.roleCode" autocomplete="off" :disabled="operationStatus === 1 || operationStatus === 2" placeholder="请输入角色标识"></el-input>
            </el-form-item>
          </el-col>
          <el-col>
            <el-form-item prop="roleDesc" label="角色描述:" :label-width="formLabelWidth">
              <el-input type="textarea" v-model="form.roleDesc" autocomplete="off" :disabled="operationStatus === 1" placeholder="请输入角色描述"></el-input>
            </el-form-item>
          </el-col>
          <el-col>
            <el-form-item prop="dsType" label="数据权限:" :label-width="formLabelWidth">
              <el-select v-model="form.dsType" placeholder="请选择数据权限" :disabled="operationStatus === 1" style="width: 100%!important;">
                <el-option v-for="item in dsTypeList" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col v-if="operationStatus === 1">
            <el-form-item label="创建时间:" :label-width="formLabelWidth">
              <el-date-picker v-model="form.createTime" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm:ss"  type="datetime" placeholder="请选择日期时间" :disabled="operationStatus === 1" style="width: 100%!important;"></el-date-picker>
            </el-form-item>
          </el-col>
        </el-form>
      </el-row>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-if="operationStatus === 4" type="primary" size="small" @click="updatePermession(roleId)">更 新</el-button>
...
export default {
  ...
  methods: {
    ...
    updatePermession(roleId) {
      const menuIds = this.$refs.menuTree.getCheckedKeys().join(',').concat(',').concat(this.$refs.menuTree.getHalfCheckedKeys().join(','))

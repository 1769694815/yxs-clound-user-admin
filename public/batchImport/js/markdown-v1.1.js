      ...
      if(params.count>1){
          ...
          if ($(insert.difficult).text().slice(3) != '简单' && $(insert.difficult).text().slice(3) != '普通' && $(insert.difficult).text().slice(3) != '困难'){
              ...
              if (diffDivTxt != '') {
                  ...
                  insert.left = "<div class='question check_error_diff' data-type='"+window.qt_type+"'>"
              }
          }
          //如果单选题、多选题的答案中有图片
...
function render_tree( jsonml ) {
  ...
  switch (tag) {
     case "title":
       ...
       // }
         tag_attrs=" class='qt_title'";
       return "<p" + tag_attrs + ">" + content.join( "" ).replace(/^\s*([0-9]+\.)|(((\()|（)[0-9]+((\))|）))/,"<span class='type-box'><span class='title type-name-"+window.qt_type+"'>$1$2</span><span class='type-name type-name-"+window.qt_type+"'>"+window.typeName[Number(window.qt_type)-1]+"</span>"+span+"</span>") + "</p>";
     break;
     case "keyA":
